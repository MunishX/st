<?php
//{"lang":"","fm_root":"\/home\/admin\/","timezone":"","date_format":"Y\/m\/d H:i:s","auth_pass":"d41d8cd98f00b204e9800998ecf8427e","error_reporting":1}

// UPDATE: Latest commit 22498c6 on Sep 18, 2021 at https://github.com/dulldusk/phpfm/blob/master/index.php

// line no. between 2927 and 2933 edited by MG (breadcrum multiple host folder fix)
//        $entry_path = DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, array_slice($entry_list,0,$x + 1)) . DIRECTORY_SEPARATOR ;

// line no. 5122 - 5135 , rewriten each() to foreach() // support for php8 by MG
...
function is_binary($file){
    ...
    fb_log($file,$mime);
    if (strpos($mime,'text') === false && strpos($mime,'javascript') === false && strpos($mime,'json') === false && strpos($mime,'xml') === false && strpos($mime,'x-empty') === false) return true;
    return false;
}
    ...
    function get_breadcrumbs($path){
        ...
        if (count($entry_list) == 1){
            ...
            for($x=0;$x<count($entry_list);$x++){
                ...
                $entry_path = implode(DIRECTORY_SEPARATOR, array_slice($entry_list,0,$x + 1)) . DIRECTORY_SEPARATOR ;
